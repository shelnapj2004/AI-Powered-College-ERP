...
// The .figma/make/site.json file this previously imported statically isn't
// part of the shipped project (only present inside the Figma Make sandbox),
// and a missing static import fails Vite's module resolution outright. Fall
// back to sane defaults so `npm run dev`/`build` work standalone; when the
// file *is* present (inside Figma Make), it's read and used as before.
type FigmaSiteConfigurationFile = Partial<FigmaSiteConfiguration>
let siteConfiguration: FigmaSiteConfigurationFile = {}
try {
  const sitePath = path.resolve(__dirname, './.figma/make/site.json')
  if (fs.existsSync(sitePath)) {
    siteConfiguration = JSON.parse(fs.readFileSync(sitePath, 'utf-8'))
  }
} catch {
  siteConfiguration = {}
}
